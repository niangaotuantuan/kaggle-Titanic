...
formula1 <- as.formula(as.factor(survived) ~ pclass  + sex + alone + fare + age)
formula2 <- as.formula(          survived  ~ pclass  + sex + alone + fare + age)
...
model.cforest <- cforest(formula2, data=clean.train, 
...
ans[ans >= 2] <- 1
ans[ans == 2] <- 1
write.csv(ans, file="submission07.csv", row.names=F)
